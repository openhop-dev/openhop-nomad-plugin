...
All events for one tag serialize. A final release published through GitHub's UI with **no assets** is built from the exact validated tag; the isolated publisher uploads both expected files without `--clobber` and reads them back byte-for-byte. An existing final release with the exact wheel/ZIP asset contract is downloaded and verified, not rebuilt. Partial/extra assets, differing bytes, moved tags, and ambiguous API errors fail closed. Only a confirmed 404 means a release is absent. The isolated publication job creates absent releases without clobbering. Do not delete authoritative source tags to recover a partial release; investigate deliberately instead.
